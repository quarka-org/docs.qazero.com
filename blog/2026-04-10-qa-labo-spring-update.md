...
And `allpv` itself grew a lot of new columns: behavioral metrics (`depth_position`, `deep_read`, `stop_max_sec`, `stop_max_pos`, `exit_pos`, `is_submit`, `dead_click_image_count`, `irritation_click_count`, `scroll_back_count`, `content_skip_count`, `exploration_count`) and generated page-type booleans (`is_article`, `is_product`, `is_form`, `is_faq`, and so on). See the [`allpv` material page](/docs/developer-manual/api/2025-10-20/materials/material-allpv-2025-10-20) for the curated list, or [`ai/materials.yaml` on GitHub](https://github.com/quarka-org/docs.qazero.com/blob/main/docs/developer-manual/api/2025-10-20/ai/materials.yaml) for the full authoritative schema.
