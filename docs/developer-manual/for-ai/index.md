...
title: For AI — Instructions & Spec
sidebar_label: For AI (Instructions & Spec)
sidebar_position: 7
description: Minimal instruction set plus the machine-readable spec for AI clients building QAL queries against the QA ZERO API.
audience: ai
---

# AI Instructions — QA ZERO API / QAL

:::note Machine-readable spec
This page is version-independent AI Instructions. The authoritative
machine-readable spec ships alongside it and is what `/guide` serves to
AI / MCP clients:

- **[`materials.yaml`](./materials.yaml)** — the full material / column
  catalogue with `since:` tags (version-independent, living).
- **`qal-validation.yaml`** — the feature-flag and validator spec, which
  is **version-pinned**. Use the copy under the version you target, e.g.
...
- `on.left` must equal **the resolved physical material name** of the
  `from` side. When `from[0]` is a view chain, the validator resolves
  the view back to its source material before comparison. Bare
  view-name prefixes (e.g. `top_pages.pv_id`) are rejected on the
  left side.
- `on.right` must equal the **`join.with` string verbatim**.

On failure, `E_INVALID_JOIN` includes structured `details` with
`side`, `received_value`, `expected_prefix`, and a `hint`. See
[Errors](../api/2026-05-11/reference/errors.md) for the exact response shape and
worked examples.
