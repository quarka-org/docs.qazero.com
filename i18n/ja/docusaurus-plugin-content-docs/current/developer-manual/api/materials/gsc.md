...
手作りサンプル。`page_id` だけを ID として表示しています (GSC には per-impression / per-session 識別子がありません)。フルスキーマは [`ai/materials.yaml`](../../for-ai/materials.yaml) にあります。
...
全リストは [`ai/materials.yaml`](../../for-ai/materials.yaml)。
...
- **[`/query` リファレンス](../2026-05-11/reference/query.md)**。
