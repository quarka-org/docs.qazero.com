...
4. **make ビュー:**
   ...
   - `sort`（**Since: 2026-04-13**）はビュー単位のブロックです: `{ "by": "<col>", "order": "asc"|"desc", "top": N? }`。`filter` / `join` / `keep` / `calc` の後に適用されます。クライアント側でのソートの代わりに使用してください。
5. **result:** 実行エンジンのランタイムホワイトリストはちょうど `use`、`limit`、`count_only`（および `x-*` アノテーション）です。**`sort` は `result` のキーではありません** — ソートは `make.<view>.sort` でビュー内に記述してください。これ以外のキーは `E_RESULT_FORBIDDEN_KEY` を発生させます。
...
- ✅ `sort`（ビュー単位、オプションの `top` 付き — **Since: 2026-04-13**）

**未実装（まだ依存しないでください）:**
- 🚧 `result.sample` / `result.include_count` / `result.return` — `result` ホワイトリストでは受理されません
- 🚧 JSON 以外のレスポンスフォーマット — 本日時点で API は常に `INLINE` / `JSON` を返します
- 🚧 フィルター条件をまたぐ `OR` 論理
...
> `pv` の降順でトップ 10 URL を取得したい場合は、`blog_by_url` ビューに `"sort": { "by": "pv", "order": "desc", "top": 10 }` を追加してください — ソートはビュー単位の関心事であり、`result` キーにはなりません。
