...
QAL クエリは、`tracking_id` と `time` でスコープを切った上で、**マテリアル** に対して 1 つ以上の **ビュー** を定義し、その中から 1 つを結果として返します。[リクエストボディの構造](#request-body-structure) の JSON 例が最小形です:

- `materials` — 読み込むマテリアル
- `time` — 時間範囲（必須）
- `make` — ビュー定義（`from`、`filter`、`join`、`keep`、`calc`）
- `result` — 返却するビュー名と、`limit` や `count_only` などのオプション

API バージョンは URL (`?version=2025-10-20`) で指定し、ボディには含めません。QAL 構文の詳細 — 全演算子、結合ルール、calc 関数、ビューチェイニング、result オプション、エラーコード — は **[QAL ガイド](./qal.md)** を参照してください。特定サーバーでどの機能が実際に有効かは、`/guide` が返す `features` マップが正本です。
